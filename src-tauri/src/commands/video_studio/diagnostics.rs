...
pub(super) fn run_process(binary: &Path, args: &[String], dir: &Path, id: &str,
    phase: &str, progress: Option<(f64, f64, f64)>, timeout: Option<Duration>, resource_id: Option<u64>) -> Result<(), String> {
    checkpoint(id)?;
    ...
    let mut reported_frames = None;
    let mut report = std::time::Instant::now();
    let mut last_progress = String::new();
    let mut advanced = std::time::Instant::now();
    let result = loop {
        if resource_id.is_some() && scheduler_poll.elapsed() >= Duration::from_secs(2) {
            // Keep file-backed diagnostics (no pipe-reader joins), but forward
            // real cumulative frame progress into the adaptive controller.
            // Unchanged or incomplete output must not refresh a stale sample.
            let text = tail(&stdout_path, 8192).unwrap_or_default();
            if let Some(frames) = latest_frames(&text) {
                if Some(frames) != reported_frames {
                    studio_hardware::report_frames(resource_id, frames);
                    reported_frames = Some(frames);
                }
            }
            ...
            if scheduler_note != note {
                update(id, |job| job.logs.push(note.clone()));
                scheduler_note = note;
            }
...
fn latest_frames(text: &str) -> Option<u64> {
    // A regular-file tail can end halfway through a writer's current line.
    // Only a newline-terminated frame counter is a complete observation.
...
mod tests {
    ...
    fn process_exit_is_logged_without_waiting_for_pipe_readers() {
        ...
        let result = run_process(Path::new("C:/Windows/System32/cmd.exe"), &args, &root, &id, "Test process", None, Some(Duration::from_secs(10)), None);
    ...
    #[test]
    fn scheduler_frame_progress_uses_latest_complete_real_counter() {
        assert_eq!(latest_frames("frame=10\r\nfps=2.0\r\nframe= 24\r\n"), Some(24));
        assert_eq!(latest_frames("frame=10\nframe=2"), Some(10));
        assert_eq!(latest_frames("frame=N/A\n"), None);
        assert_eq!(latest_frames("frame=-1\n"), None);
        assert_eq!(latest_frames("rame=50\nprogress=continue\n"), None);
        assert_eq!(latest_frames("frame=10\nframe=N/A\n"), None);
    }
}
