...
try {
  ...
  const workflow = page.getByRole("navigation", { name: "Video editing workflow" });
  await workflow.getByRole("link", { name: /^Titles & graphics/ }).click();
  const opening = page.getByRole("region", { name: "Opening title", exact: true });
  await opening.getByRole("textbox", { name: "Opening title", exact: true }).waitFor();
  const openingHeading = opening.getByRole("textbox", { name: "Heading (optional)", exact: true });
  assert.equal(await openingHeading.inputValue(), "", "old projects start with no hardcoded heading");
  await openingHeading.fill("SYDNEY POLO CHAMPIONSHIPS");
  await opening.locator(".studio-graphics-eyebrow").getByText("SYDNEY POLO CHAMPIONSHIPS", { exact: true }).waitFor();
  assert.equal((await saved()).titleHeading, "SYDNEY POLO CHAMPIONSHIPS");
  assert.deepEqual((await saved()).clips, initial.clips, "opening heading keeps prepared clips");
  await openingHeading.fill("");
  assert.equal(await opening.locator(".studio-graphics-eyebrow").count(), 0, "blank heading hides the top line without fallback");
  assert.equal(await page.getByRole("textbox", { name: "Opening title", exact: true }).count(), 1, "one opening editor, not duplicate project controls");
  await page.getByRole("button", { name: "Edit opening title", exact: true }).click();
  await page.waitForFunction(() => document.activeElement?.id === "studio-opening-heading");
  assert.deepEqual((await saved()).clips, initial.clips, "navigation never adds a synthetic clip or changes its cache");
  const pinned = page.getByRole("button", { name: "Edit opening title", exact: true });
  const openingText = opening.getByRole("textbox", { name: "Opening title", exact: true });
  const openingMode = opening.getByRole("combobox", { name: "Opening title style", exact: true });
  const openingSeconds = opening.getByRole("spinbutton", { name: /^Opening title duration/ });
  const plannedChapters = page.getByRole("list", { name: "Ordered chapters", exact: true });
  await openingText.fill("SUMMER CHAMPIONSHIP");
  await opening.getByRole("textbox", { name: "Subtitle / event date", exact: true }).fill("Final round · 26 September");
  await openingSeconds.fill("7");
  await pinned.getByText("Opening card · 7 seconds · adds 7 seconds", { exact: true }).waitFor();
  await plannedChapters.getByRole("listitem").first().getByText("≈00:00:07", { exact: true }).waitFor();
  await openingMode.selectOption("overlay");
  await pinned.getByText("Opening overlay · first clip · no added time", { exact: true }).waitFor();
  await plannedChapters.getByRole("listitem").first().getByText("≈00:00:00", { exact: true }).waitFor();
  await openingMode.selectOption("none");
  await pinned.getByText("Opening title · Off", { exact: true }).waitFor();
  assert.equal(await openingText.inputValue(), "SUMMER CHAMPIONSHIP", "Off keeps configured text");
  assert.equal(await opening.getByRole("button", { name: "Rendered preview", exact: true }).isDisabled(), true);
  await openingMode.selectOption("card");
  await openingText.fill("");
  await pinned.getByText("Opening title · Add title text", { exact: true }).waitFor();
  await openingText.fill(initial.title);
  await openingSeconds.fill("0");
  await pinned.getByText("Opening title · Off", { exact: true }).waitFor();
  await openingSeconds.fill(String(initial.titleSeconds));
  await opening.getByRole("textbox", { name: "Subtitle / event date", exact: true }).fill(initial.subtitle);
  await openingMode.selectOption(initial.openingTitleMode);
  assert.deepEqual((await saved()).clips, initial.clips, "opening text, mode and duration preserve every picture approval, revision and cache");
  await page.getByRole("searchbox", { name: "Search clips", exact: true }).fill("no such clip");
  assert.equal(await page.locator(".studio-sequence-row").count(), 0);
  assert.equal(await pinned.isVisible(), true, "opening entry survives clip filtering");
  await pinned.focus();
  await page.keyboard.press("Enter");
  await page.waitForFunction(() => document.activeElement?.id === "studio-opening-heading");
  await page.getByRole("searchbox", { name: "Search clips", exact: true }).fill("");
  assert.equal(await page.locator(".studio-sequence-row").count(), initial.clips.length);
  const selectedClipBefore = await page.locator(".studio-clip-select[aria-current='true']").textContent();
  await opening.getByRole("button", { name: "Edit shared graphics defaults", exact: true }).click();
  await page.waitForFunction(() => document.activeElement?.id === "studio-project-toggle-graphics");
  const projectControls = page.getByRole("region", { name: "Project settings", exact: true });
  assert.equal(await projectControls.getByRole("button", { name: "Graphics", exact: true }).getAttribute("aria-expanded"), "true");
  await projectControls.getByRole("button", { name: "Preview opening title in Titles & graphics", exact: true }).click();
  await page.waitForFunction(() => document.activeElement?.id === "studio-opening-heading");
  assert.equal(await page.locator(".studio-clip-select[aria-current='true']").textContent(), selectedClipBefore, "presentation navigation retains the selected clip");
  await projectControls.getByRole("button", { name: "Graphics", exact: true }).click();
  for (const [width, height] of [[1360, 1000], [1920, 1080], [360, 800]]) {
    await page.setViewportSize({ width, height });
    await opening.scrollIntoViewIfNeeded();
    assert.equal(await page.locator("main").evaluate((node) => node.scrollWidth <= node.clientWidth + 1), true, `opening editor fits at ${width}px`);
    if (width === 360) {
      await pinned.click();
      await page.waitForFunction(() => {
        const top = document.getElementById("studio-opening-heading").getBoundingClientRect().top;
        const main = document.querySelector("main").getBoundingClientRect();
        return top >= main.top && top < main.top + 30;
      });
      await page.screenshot({ path: `${output}/opening-title-${width}.png` });
      await opening.getByRole("button", { name: "Rendered preview", exact: true }).scrollIntoViewIfNeeded();
      await page.screenshot({ path: `${output}/opening-preview-${width}.png` });
    } else await opening.screenshot({ path: `${output}/opening-title-${width}.png` });
  }
  ...
  const clipHeading = titles.getByRole("textbox", { name: "Heading (optional)", exact: true });
  const clipSubtitle = titles.getByRole("textbox", { name: "Subtitle (optional)", exact: true });
  assert.equal(await clipHeading.inputValue(), "");
  assert.equal(await titles.locator(".studio-graphics-eyebrow").count(), 0, "no hardcoded clip heading");
  await clipHeading.fill("ROUND ONE"); await clipSubtitle.fill("72 points · First place");
  assert.equal((await saved()).clips[0].titleHeading, "ROUND ONE");
  assert.equal((await saved()).clips[0].titleSubtitle, "72 points · First place");
  await titles.locator(".studio-graphics-eyebrow").getByText("ROUND ONE", { exact: true }).waitFor();
  await titles.getByRole("button", { name: "Rendered preview", exact: true }).click();
  await titles.getByRole("img", { name: "Rendered title preview", exact: true }).waitFor();
  assert.equal(await page.evaluate(() => window.__graphicsCalls.at(-1).project.clips[0].titleHeading), "ROUND ONE");
  assert.equal(await page.evaluate(() => window.__graphicsCalls.at(-1).project.clips[0].titleSubtitle), "72 points · First place");
  await clipHeading.fill("");
  ...
  await page.getByRole("button", { name: "Open project", exact: true }).click();
  const readyClipsBeforeTitle = (await saved()).clips;
  await workflow.getByRole("link", { name: /^Titles & graphics/ }).click();
  await openingText.fill("Updated opening only");
  await opening.getByRole("textbox", { name: "Heading (optional)", exact: true }).fill("SUMMER FINALS");
  await openingSeconds.fill("6");
  assert.deepEqual((await saved()).clips, readyClipsBeforeTitle, "saved ready project keeps caches after opening edit");
  await exportButton.click();
  ...
  assert.equal(await page.evaluate(() => window.__lastStudioRequest.project.title), "Updated opening only");
  assert.equal(await page.evaluate(() => window.__lastStudioRequest.project.titleHeading), "SUMMER FINALS");
  assert.equal(await page.evaluate(() => window.__lastStudioRequest.project.titleSeconds), 6);
  await page.getByText(/Finishing-only export queued/).waitFor();
  await opening.getByRole("button", { name: "Rendered preview", exact: true }).click();
  await opening.getByRole("img", { name: "Rendered title preview", exact: true }).waitFor();
  assert.equal(await page.evaluate(() => window.__graphicsCalls.at(-1).target), "opening");
  await openingText.fill("Updated again");
  assert.equal(await opening.getByRole("img").count(), 0, "opening edit clears rendered preview");
  await page.evaluate(() => { window.__graphicsDeferred = true; });
  await opening.getByRole("button", { name: "Rendered preview", exact: true }).click();
  await page.waitForFunction(() => window.__graphicsPending.length === 1);
  ...
  await page.evaluate(() => window.__graphicsPending.shift()());
  assert.equal(await opening.getByRole("img").count(), 0, "opening preview from the old project cannot enter its replacement");
  assert.equal(await pinned.isVisible(), true, "empty project still has an opening-title entry");
  assert.equal(await openingText.isEnabled(), true, "no clip selection gate on configuring an opening");
  assert.deepEqual(failures, []);
  console.log("PASS: single opening editor, navigation/focus/defaults, card-overlay-off timing, filtered/empty sequences, opening and scorecard cache isolation, saved-project finishing-only dispatch, chapters, previews/stale responses, keyboard and 360/1360/1920 layouts.");
